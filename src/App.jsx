...
function App() {
  ...
  const [ranking, setRanking] = useState([]) // opcional: mostrar ranking depois

  const perguntas = [
    { id: 1, pergunta: 'Em que era geológica está o Período Quaternário?', opcoes: [
      { texto: 'Mesozoica', correta: false },
      { texto: 'Cenozoica', correta: true },
      { texto: 'Paleozoica', correta: false },
      { texto: 'Arqueana', correta: false },
    ]},
    { id: 2, pergunta: 'Qual é a primeira época do Quaternário?', opcoes: [
      { texto: 'Holoceno', correta: false },
      { texto: 'Pleistoceno', correta: true },
      { texto: 'Triássico', correta: false },
      { texto: 'Jurássico', correta: false },
    ]},
    { id: 3, pergunta: 'Qual espécie humana surgiu há cerca de 300 mil anos?', opcoes: [
    ...
    { id: 7, pergunta: 'O que permitiu o surgimento das primeiras civilizações no Holoceno?', opcoes: [
    ...
    { id: 8, pergunta: 'O que caracteriza o Antropoceno?', opcoes: [
    ...
    { id: 9, pergunta: 'Como as glaciações influenciaram o deslocamento humano no Pleistoceno?', opcoes: [
    ...
    { id: 10, pergunta: 'Qual a relação entre a extinção da megafauna e a chegada do Homo sapiens a novos territórios?', opcoes: [
    ...
    { id: 11, pergunta: 'Durante o Pleistoceno, qual foi uma das principais adaptações humanas para sobreviver ao frio?', opcoes: [
    ...
    { id: 12, pergunta: 'Qual foi uma das primeiras espécies de animais domesticados pelo ser humano no Holoceno?', opcoes: [
      { texto: 'Gato', correta: false },
      { texto: 'Cavalo', correta: false },
      { texto: 'Cão', correta: true },
      { texto: 'Galinha', correta: false },
    ]},
    { id: 13, pergunta: 'O que marcou a transição do Pleistoceno para o Holoceno?', opcoes: [
      { texto: 'Extinção dos dinossauros', correta: false },
    ...
    { id: 14, pergunta: 'O termo "Antropoceno" é usado para indicar:', opcoes: [
    ...
    { id: 15, pergunta: 'Qual inovação cultural no Holoceno permitiu maior organização social e surgimento de vilas e cidades?', opcoes: [
  ...
  async function handleFinish({ score, total }) {
    ...
    try {
      const res = await fetch('/salvar_resultado', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(payload),
      })
      const json = await res.json()
      if (!res.ok || !json.sucesso) throw new Error(json.erro || 'Falha ao enviar')
      alert('Enviado! id: ' + json.id)
      // (opcional) recarregar ranking:
      carregarRanking()
    } catch (err) {
  ...
  async function carregarRanking() {
    try {
      const res = await fetch('/ranking?limit=10')
  ...
  // ======= TELA DO QUIZ (depois de começar) =======
  if (started) {
    return (
      <>
        <header>
          <div className={style.containerDna}>
            {/* use arquivos de public/ com caminho absoluto */}
        ...
        <div className={style.containerIcon}>
          <div className={style.icon}><img src="/cerebro.png" alt="Icone de Cerebro" /></div>
          <h1>Quiz de Evolução <br/>Humana</h1>
          <p>Boa sorte, {name}!</p>
        </div>

        {/* Renderiza o quiz e envia ao finalizar */}
        <Random perguntas={perguntas} nome={name} onFinish={handleFinish} />
  ...
  return (
    <>
      ...
      <div className={style.containerForm}>
        {/* Tornar o botão funcional com onSubmit */}
        <form className={style.form} onSubmit={handleStart}>
          <label htmlFor="inome">
